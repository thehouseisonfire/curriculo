...
\item[\textbf{Ferramentas}] Git, GitHub Actions, Docker, Docker Compose, Supabase, Firebase, Vim
\end{itemize}
% \end{multicols}
...
\begin{resume_list}
\item Projetei e desenvolvi um ERP para uso na operação interna da empresa, abrangendo compra e venda de produtos, simulação e geração de tabelas de preços, gerenciamento de rotas, clientes e funcionários, calendário e planejamento de rotas, gestão e histórico de comprovantes de entrega e devoluções, relatórios operacionais e KPIs, API externa para integração de sistemas, portais para fornecedores e quadro Kanban para gerenciamento de pedidos.
\item Desenvolvi uma aplicação de e-commerce para a distribuidora utilizando Next.js, Supabase, Medusa.js, shadcn/ui, Tailwind CSS v4 e o SDK do Mercado Pago. Também iniciei o desenvolvimento de um aplicativo mobile integrado à plataforma.
...
\item Implementei diversas funcionalidades e regras de negócio para a ferramenta interna de cálculo e geração de contratos, incluindo persistência de dados entre telas, cálculo inverso a partir do valor final, validação de dados e suporte a múltiplos ambientes por contrato.
\item Realizei engenharia reversa de artefatos gerados pelo software CAD PROMOB para extração direta de dados do XML, além de implementar concatenação de PDFs, corrigir diversos erros lógicos e obter uma melhoria de aproximadamente 10x no tempo de criação de contratos.
...
\begin{resume_list}
\item Biblioteca completa de cliente MQTT, baseada em um fork criado após a empresa original cessar o desenvolvimento aberto do projeto.
\item Escrita em Rust, utilizando o runtime Tokio e Flume para comunicação MPSC.
\item Projeto amplamente utilizado, com mais de 250 mil downloads.
\end{resume_list}
